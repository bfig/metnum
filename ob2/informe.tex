...
%Ej2
Se presnetan dos tablas de mediciones para las funciones no lineales $g_1$ y $g_2$ 


se definen :\\
$g_1(x)=c x^{-p} $ y $g_2(x)=d x^{-q} $

se convienten los problemas a PMCL y se resuven con método de Ecuaciones Normales y método de descomposición QR. Se resuelve el PMCNL con el método de Gauss-Newton.
...
\clearpage
\section{Estudio Experimental}\label{Comparison}
